...
import HybridMoodBoard from "./HybridMoodBoard";
...
export default function VisualIdentityPanel({
        ...
        <div className="rounded-2xl border overflow-hidden"
          ...
          <div className="p-6">
            ...
            {generatingMoodboard ? (
              ...
              <div className="rounded-xl h-full flex flex-col items-center justify-center gap-4"
                ...
                <p className="text-xs text-center px-8" style={{ color: "var(--color-text-hint)" }}>
                    Building your brand system from <em>{intake.color_mood || "your brand colours"}</em> and adding AI texture studies                </p>
              </div>
            ) :  (
               <HybridMoodBoard
                brandName={brandName}
                ...
                pal={pal}
                textures={visuals.mood_board ?? []}
                logoProfile={visuals.logo_profile}
                logoApp={visuals.logo_app}
                logoBusiness={visuals.logo_business}
                logoFallback={(t) => <LogoMark type={t} size={60} />}
            ...
            <div className="mt-3 px-4 py-2 rounded-lg flex items-start gap-2"
              ...
              <span style={{ color: "var(--color-pulse)", fontSize: "0.85rem", lineHeight: 1 }}>✦</span>
              <p className="text-xs leading-relaxed" style={{ color: "rgba(230,232,240,0.65)" }}>
                <><strong style={{ color: "#c4b5fd" }}>AI brand system</strong> — palette, typography, and logo lockups built from AI-generated brand DNA, with AI texture studies.</>
              </p>
            </div>
  ...
  const hasMockup = (() => {
    ...
    if (!html || html.trim().length < 800) return false;
    const lower = html.toLowerCase();

    const hasBrandName = lower.includes(brandName.toLowerCase());
    const hasHeading = lower.includes("<h1") || lower.includes("<h2");

    // Completeness markers. The brand name and heading appear early, so a
    // truncated response passes those checks while missing the right column
    // and bottom strip entirely. Require content that only appears at the end.
    const closed = lower.includes("</body>") || lower.trimEnd().endsWith("</html>");
    const audience = (intake.target_audience || "").trim().toLowerCase();
    const hasBottomStrip = audience.length > 0 ? lower.includes(audience.slice(0, 12)) : true;

    return hasBrandName && hasHeading && closed && hasBottomStrip;
  })();
...
function CSSLandingPageFallback({
  ...
  card: NameCard;
  intake: IntakePayload;
  pal: { bg: string; accent: string; text: string; accent2: string };
  selectedLogoType: string;
  visuals: VisualsResponse;
  LogoMark: (p: { type: string; size?: number; showTagline?: boolean }) => React.ReactElement;
}) {
...
function StepTabs({
  ...
  visualStep: VisualStep;
  setVisualStep: (s: VisualStep) => void;
  selectedLogoType: string;
}) {
